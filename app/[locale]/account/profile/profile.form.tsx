...
export const ProfileForm = (props: TFormProps): JSX.Element => {
  ...
  return (
    <form
      className="flex flex-col space-y-8 divide-y divide-gray-200 md:w-account-form-md"
      noValidate
      action={formAction}
    >
        ...
        {provider !== EOAuthProvider.CREDENTIALS ? (
          <>
            ...
            <input
              ...
              id="name"
              name="name"
              className={`${state?.data?.errors?.name || state?.status === EHttpResponseStatus.ERROR ? "invalid" : ""}`}
              value={name ?? ""}
              readOnly
              required
            />
        ...
        <div className="grid md:grid-cols-[2fr_1fr] md:space-x-2 rtl:md:space-x-reverse space-y-2 md:space-y-0">
          <div className="flex flex-col">
            ...
            <input
              ...
              id="city"
              name="city"
              className={`${state?.data?.errors?.city || state?.status === EHttpResponseStatus.ERROR ? "invalid" : ""}`}
              value={city ?? ""}
              required
