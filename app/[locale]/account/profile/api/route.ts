...
import { getUserById } from "@/app/[locale]/_data"
...
export const POST = async <T extends Partial<User> & { birth_date: string; current_password: string } & UserAddress>(
  ...
  if (body.current_password && body.password && existingUser.password) {
    const isPasswordValid = await compare(body.current_password, existingUser.password)
